function [Z P Header] = rp_TFRtoZ(ImgFile,OutputName,Flag,Df1,Df2,Header)
...
if ischar(ImgFile)
    [Data VoxelSize Header]=rp_readfile(ImgFile);
else
    Data = ImgFile;
...
if ~strcmpi(OutputName,'DO NOT OUTPUT IMAGE')%Added by Sandy to make it compatible with Image matrix
    rp_writefile(Z,OutputName,[nDim1,nDim2,nDim3],VoxelSize, Header,'double');
end
